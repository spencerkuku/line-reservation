...
LINE 預約系統採用前後端分離架構，透過 RESTful API 進行通訊。系統由三個主要部分組成：

1. **前端管理介面** (Vue.js SPA)
2. **後端 API 服務** (Laravel)
3. **LINE Messaging API** (第三方服務)

### 核心特性

- 🔄 **前後端分離**：獨立開發、部署、擴展
- 🔐 **Token 認證**：Laravel Sanctum 提供安全的 API 認證
- 📱 **響應式設計**：支援桌面、平板、手機
- 🤖 **LINE Bot 整合**：即時訊息處理與推播
- 🗄️ **關聯式資料庫**：MySQL 保證資料一致性
