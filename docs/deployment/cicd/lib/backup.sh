...
get_backup_dir() {
    ...
    local base_dir="${BACKUP_DIR:-$HOME/backups/line-reservation}"
    
    case "$backup_type" in
        env)      echo "$base_dir/env" ;;
        project)  echo "$base_dir/project" ;;
        db)       echo "$base_dir/db" ;;
...
restore_env() {
    ...
    if [ -f "$extracted_dir/backend.env" ]; then
        cp "$extracted_dir/backend.env" "$project_dir/backend/.env"
        log_info "已恢復 backend/.env"
    fi
    
    if [ -f "$extracted_dir/frontend.env" ]; then
        cp "$extracted_dir/frontend.env" "$project_dir/frontend/.env"
        log_info "已恢復 frontend/.env"
    fi
    
    if [ -f "$extracted_dir/db_credentials.txt" ]; then
        cp "$extracted_dir/db_credentials.txt" "$project_dir/db_credentials.txt"
...
restore_full() {
    ...
    # 恢復資料庫
    local db_file=$(find "$extracted_dir" -name "db_backup_*.sql*" | head -1)
    if [ -n "$db_file" ]; then
...
create_backup() {
    ...
    case "$backup_type" in
        ...
        project)
            ...
            ;;
        db)
            backup_database "$(get_backup_dir db)"
            ;;
        full)
        ...
        *)
            ...
            log_info "可用類型: env, project, db, full"
            return 1
            ;;
    esac
}
...
# ===== 備份清理 =====
cleanup_old_backups() {
    local backup_dir="${1:-$(get_backup_dir)}"
    local retention_days="${2:-30}"
    
    log_step "清理超過 $retention_days 天的備份..."
    
    local count=$(find "$backup_dir" -type f -name "*.tar.gz" -mtime +$retention_days 2>/dev/null | wc -l)
    
    if [ "$count" -gt 0 ]; then
        log_info "找到 $count 個過期備份"
        
        if confirm_action "確定要刪除這些備份嗎?"; then
            find "$backup_dir" -type f -name "*.tar.gz" -mtime +$retention_days -delete
            log_success "已刪除 $count 個過期備份"
        fi
    else
        log_info "沒有需要清理的備份"
    fi
...
show_backup_status() {
    ...
    echo ""
    echo "🗄️ 資料庫備份:"
    list_db_backups 2>/dev/null || echo "  (需要載入 db 模組)"
    
    echo ""
    local total_size=$(du -sh "$(get_backup_dir)" 2>/dev/null | cut -f1)
...
get_latest_backup() {
    ...
    case "$backup_type" in
        env)
            ls -t "$backup_dir"/env_backup_*.tar.gz 2>/dev/null | head -1
            ;;
        project)
            ls -t "$backup_dir"/project_backup_*.tar.gz 2>/dev/null | head -1
            ;;
        db)
            ls -t "$backup_dir"/db_backup_*.sql* 2>/dev/null | head -1
            ;;
        full)
            ls -t "$backup_dir"/full_backup_*.tar.gz 2>/dev/null | head -1
            ;;
    esac
}
...
count_backups() {
    ...
    case "$backup_type" in
        env)
            ls "$backup_dir"/env_backup_*.tar.gz 2>/dev/null | wc -l
            ;;
        project)
            ls "$backup_dir"/project_backup_*.tar.gz 2>/dev/null | wc -l
            ;;
        db)
            ls "$backup_dir"/db_backup_*.sql* 2>/dev/null | wc -l
            ;;
        full)
            ls "$backup_dir"/full_backup_*.tar.gz 2>/dev/null | wc -l
            ;;
    esac
}
